...
function isIdempotentGetRequest(
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const isAuthEndpoint =
      /\/auth\/(login|logout|register|forgot-password|reset-password|refresh|google\/exchange)/.test(
        requestUrl,
      );
...
export const apiService = {
  ...
  getMachineTimelineSummary: (
    machineId: string,
    options?: { signal?: AbortSignal },
  ) =>
    api.get(`/machines/${machineId}/timeline/summary`, {
  ...
  getMachineTimeline: (
    ...
    api.get(`/machines/${machineId}/timeline`, {
      params,
      signal: options?.signal,
    }),
  ...
  updateDevice: (id: string, data: AnyObject) =>
  ...
  resolveFaultEvent: (id: string) =>
    api.patch(`/live-monitoring/faults/${id}/resolve`),

  // Machine Types
  getMachineTypes: (params?: PaginationParams) =>
    api.get("/machine-types", withPagination(params)),
  createMachineType: (data: AnyObject) => api.post("/machine-types", data),
  updateMachineType: (id: string, data: AnyObject) =>
    api.patch(`/machine-types/${id}`, data),
  deleteMachineType: (id: string) => api.delete(`/machine-types/${id}`),

  // Module Types
  getModuleTypes: (params?: PaginationParams) =>
    api.get("/module-types", withPagination(params)),
  createModuleType: (data: AnyObject) => api.post("/module-types", data),
  updateModuleType: (id: string, data: AnyObject) =>
    api.patch(`/module-types/${id}`, data),
  deleteModuleType: (id: string) => api.delete(`/module-types/${id}`),

  // Modules
  getModules: (params?: PaginationParams) =>
    api.get("/modules", withPagination(params)),
  createModule: (data: AnyObject) => api.post("/modules", data),
  updateModule: (id: string, data: AnyObject) =>
  ...
  // Catalogues
  getCatalogues: (params?: PaginationParams) =>
    api.get("/catalogues", withPagination(params)),
  createCatalogue: (data: AnyObject) => api.post("/catalogues", data),
  updateCatalogue: (id: string, data: AnyObject) =>
    api.patch(`/catalogues/${id}`, data),
  deleteCatalogue: (id: string) => api.delete(`/catalogues/${id}`),

  // Stocks
  getStocks: (params?: PaginationParams) =>
    api.get("/stocks", withPagination(params)),
  createStock: (data: AnyObject) => api.post("/stocks", data),
  updateStock: (id: string, data: AnyObject) =>
  ...
  // OT Pieces
  getOtPieces: (params?: PaginationParams) =>
    api.get("/ot-pieces", withPagination(params)),
  createOtPiece: (data: AnyObject) => api.post("/ot-pieces", data),
  requestOperatorParts: (
    workOrderId: string,
    data: { part_id: string; quantity: number },
  ) => api.post(`/operator/work-orders/${workOrderId}/parts-request`, data),
  updateOtPiece: (id: string, data: AnyObject) =>
    api.patch(`/ot-pieces/${id}`, data),
  deleteOtPiece: (id: string) => api.delete(`/ot-pieces/${id}`),

  // Lubrifiants and logs
  getLubrifiants: (params?: PaginationParams) =>
    api.get("/lubrifiants", withPagination(params)),
  createLubrifiant: (data: AnyObject) => api.post("/lubrifiants", data),
  updateLubrifiant: (id: string, data: AnyObject) =>
    api.patch(`/lubrifiants/${id}`, data),
  deleteLubrifiant: (id: string) => api.delete(`/lubrifiants/${id}`),
  getLubrificationLogs: (params?: PaginationParams) =>
    api.get("/lubrification-logs", withPagination(params)),
  createLubrificationLog: (data: AnyObject) =>
    api.post("/lubrification-logs", data),
  updateLubrificationLog: (id: string, data: AnyObject) =>
    api.patch(`/lubrification-logs/${id}`, data),
  deleteLubrificationLog: (id: string) =>
    api.delete(`/lubrification-logs/${id}`),

  // KPI
  getKpis: (params?: PaginationParams) =>
    api.get("/kpis", withPagination(params)),
  ...
  getCalendarNotifications: () =>
    api.get("/work-orders/calendar/notifications"),
  getCalendarEventDetails: (id: string) =>
    api.get(`/work-orders/calendar/event/${id}`),
  ...
  getMyCalendarEventDetails: (id: string) =>
    api.get(`/operator/calendar/events/${id}`),
  startMyCalendarEvent: (id: string) =>
    api.post(`/operator/calendar/events/${id}/start`),
  completeMyCalendarEvent: (id: string) =>
    api.post(`/operator/calendar/events/${id}/complete`),
  ...
  reviewTechnicianWorkOrder: (id: string, action: "return" | "intervene") =>
    api.patch(`/technician/work-orders/${id}/review`, { action }),
  startTechnicianWorkOrder: (id: string) =>
    api.patch(`/technician/work-orders/${id}/start`),
  waitForTechnicianParts: (id: string) =>
    api.patch(`/technician/work-orders/${id}/waiting-parts`),
  resumeTechnicianWorkOrder: (id: string) =>
    api.patch(`/technician/work-orders/${id}/resume`),
  updateTechnicianReport: (id: string, data: AnyObject) =>
    api.patch(`/technician/work-orders/${id}/report`, data),
  setTechnicianPartQuantity: (
    id: string,
  ...
  // Capteurs (Sensors)
  getCapteurs: (params?: PaginationParams) =>
    api.get("/capteurs", withPagination(params)),
  createCapteur: (data: AnyObject) => api.post("/capteurs", data),
  updateCapteur: (id: string, data: AnyObject) =>
    api.patch(`/capteurs/${id}`, data),
  deleteCapteur: (id: string) => api.delete(`/capteurs/${id}`),

  // Sensor measurements
  getMesures: (params?: FilterPaginationParams) =>
    api.get("/mesures", { params }),
  createMesure: (data: AnyObject) => api.post("/mesures", data),
  updateMesure: (id: string, data: AnyObject) =>
    api.patch(`/mesures/${id}`, data),
  deleteMesure: (id: string) => api.delete(`/mesures/${id}`),

  // Standard parts assigned to module types
  getModulePieces: (params?: FilterPaginationParams) =>
    api.get("/module-pieces", { params }),
  createModulePiece: (data: AnyObject) => api.post("/module-pieces", data),
  updateModulePiece: (id: string, data: AnyObject) =>
    api.patch(`/module-pieces/${id}`, data),
  deleteModulePiece: (id: string) => api.delete(`/module-pieces/${id}`),

  // Persistent preventive checklist tasks
  getPreventiveTasks: (params?: FilterPaginationParams) =>
    api.get("/preventive-tasks", { params }),
  createPreventiveTask: (data: AnyObject) =>
    api.post("/preventive-tasks", data),
  updatePreventiveTask: (id: string, data: AnyObject) =>
    api.patch(`/preventive-tasks/${id}`, data),
  ...
  updateInterventionReport: (id: string, data: AnyObject) =>
    api.patch(`/intervention-reports/${id}`, data),
  deleteInterventionReport: (id: string) =>
  ...
  getPanneSolutions: (params?: PaginationParams) =>
    api.get("/panne-solutions", withPagination(params)),
  getOperatorFaultSolutions: (params?: AnyObject) =>
    api.get("/operator/fault-solutions", { params }),
  createPanneSolution: (data: AnyObject) => api.post("/panne-solutions", data),
  updatePanneSolution: (id: string, data: AnyObject) =>
    api.patch(`/panne-solutions/${id}`, data),
  deletePanneSolution: (id: string) => api.delete(`/panne-solutions/${id}`),

  getDocuments: (params?: PaginationParams) =>
    api.get("/documents", withPagination(params)),
  getDocumentsByMachine: (machineId: string) =>
    api.get(`/documents/machine/${machineId}`),
  getOperatorManuals: (params?: AnyObject) =>
    api.get("/operator/manuals", { params }),

  getDocument: (id: string) => api.get(`/documents/${id}`),

  createDocument: (data: AnyObject) => api.post("/documents", data),

  updateDocument: (id: string, data: AnyObject) =>
  ...
  getKnowledgeArticleVersions: (id: string) =>
    api.get(`/knowledge-base/articles/${id}/versions`),

  createKnowledgeArticle: (data: AnyObject) =>
    api.post("/knowledge-base/articles", data),
  ...
  deleteKnowledgeArticle: (id: string) =>
    api.delete(`/knowledge-base/articles/${id}`),
  ...
  getMyCalendarEvents: (params?: AnyObject) =>
    api.get("/operator/calendar/my", { params }),
  ...
  scheduleOperatorPreventive: (data: {
    machine_id: string;
    plan_id: string;
  ...
  batchDynamicTranslations: (
    ...
    options?: { signal?: AbortSignal },
  ) => api.post("/translations/batch", data, { signal: options?.signal }),
  ...
  getSavedViews: (pageKey: string) =>
    api.get("/saved-views", { params: { pageKey } }),
  ...
  bulkApproveUsers: (userIds: string[]) =>
    api.post("/users/bulk-approve", { userIds }),
