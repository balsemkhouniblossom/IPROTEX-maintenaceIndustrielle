...
import {
  ...
  parseWorkbook,
  type SheetPreview,
} from "@/services/spreadsheetPreview";
...
export default function SpreadsheetViewer({
  ...
  const previewResult = useMemo((): {
    ...
    error: "tooLarge" | "renderer" | null;
  } => {
    if (!workbook || !activeSheet) return { preview: null, error: null };
    try {
      return { preview: buildSheetPreview(workbook, activeSheet), error: null };
    } catch (error) {
      const errorKind = error instanceof RangeError ? "tooLarge" : "renderer";
      return { preview: null, error: errorKind };
    }
  ...
  if (!workbook) {
    return (
      <div className="flex min-h-[32vh] items-center justify-center text-sm text-slate-600">
    ...
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      >
        {workbook.SheetNames.map((sheetName) => (
          <button
            ...
            type="button"
            role="tab"
            aria-selected={activeSheet === sheetName}
            className={`shrink-0 rounded-md px-3 py-1.5 text-sm font-medium ${activeSheet === sheetName ? "bg-blue-700 text-white" : "bg-white text-slate-700 hover:bg-slate-100"}`}
            onClick={() => {
      ...
      {previewResult.error === "tooLarge" ? (
        ...
        <>
          ...
          <div
            ref={scrollRef}
            className="relative h-[60vh] max-h-[640px] overflow-auto"
            role="grid"
            aria-label={t("spreadsheetSheet", { sheet: preview.name })}
          ...
          >
            <div
              ...
              style={{
                ...
                width: preview.columnCount * COLUMN_WIDTH,
              }}
            >
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <div
                    ...
                    className="absolute left-0 top-0 grid border-b border-slate-200 bg-white"
                    style={{
                      width: preview.columnCount * COLUMN_WIDTH,
                      height: virtualRow.size,
                      transform: `translateY(${virtualRow.start}px)`,
                      gridTemplateColumns: `repeat(${preview.columnCount}, ${COLUMN_WIDTH}px)`,
                    }}
                  >
                    {Array.from(
                      ...
                      (_, columnIndex) => {
                        ...
                        return (
                          <div
                            ...
                            className="truncate border-e border-slate-200 px-2 py-2 text-sm text-slate-800"
                            style={{
                              gridColumn: `span ${preview.horizontalMerges.get(key) ?? 1}`,
                            }}
