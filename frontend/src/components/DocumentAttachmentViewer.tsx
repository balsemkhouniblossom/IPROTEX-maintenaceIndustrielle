...
const SpreadsheetViewer = dynamic(
  () => import("@/components/SpreadsheetViewer"),
  { ssr: false },
);
...
export default function DocumentAttachmentViewer({
  ...
  const [contentFailure, setContentFailure] = useState<ContentFailure | null>(
    null,
  );
  const viewerKind = getAttachmentViewerKind(document);
  const sourceUrl = useMemo(
    () => resolveAttachmentViewerUrl(document),
    [document],
  );
  ...
  if (state.status === "failure") {
    return (
      <ViewerMessage
        ...
        action={
          state.reason === "unauthorized" ? undefined : (
            <button
              type="button"
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">{downloadAction}</div>
      <ErrorBoundary
        boundaryName={`document-${viewerKind}-viewer`}
...
function ViewerMessage({
  ...
  message,
  action,
}: Readonly<{ icon: ReactNode; message: string; action?: ReactNode }>) {
  return (
    <output className="flex min-h-[32vh] flex-col items-center justify-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-6 text-center text-slate-600">
      {icon}
...
function isBackendDocumentFileUrl(url: string): boolean {
  ...
  if (normalizedUrl.startsWith("/documents/"))
    return /\/documents\/[^/]+\/file$/.test(normalizedUrl);
  return (
    normalizedUrl.startsWith(`${normalizedBase}/documents/`) &&
    /\/documents\/[^/]+\/file$/.test(normalizedUrl.slice(normalizedBase.length))
  );
}
