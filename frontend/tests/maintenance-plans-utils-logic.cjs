...
} = require('../src/app/[locale]/maintenance-plans/utils.ts');
...
test('getNextFieldValue returns currentValue when custom and in options', () => {
  assert.equal(getNextFieldValue(['a', 'b'], 'a', CUSTOM_OPTION), 'a');
});

test('getNextFieldValue returns empty when custom and not in options', () => {
  assert.equal(getNextFieldValue(['a', 'b'], 'c', CUSTOM_OPTION), '');
});
...
test('frequencyTranslationKey returns null for invalid frequency', () => {
  ...
  assert.equal(frequencyTranslationKey(1.5, 'day'), null);
});
