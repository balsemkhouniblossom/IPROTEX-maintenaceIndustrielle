...
describe('TrainingSampleService', () => {
  function buildService(
    ...
    const machineModel = {
      find: jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(machines) }),
      }),
    };
    ...
    return new TrainingSampleService(machineModel as never, featureExtractionService as never);
  }
  ...
  it('generates 12 weekly checkpoints per machine', async () => {
    ...
    expect(samples.every((s) => s.machineId === machine._id.toString())).toBe(true);
  });
  ...
  it('keeps a checkpoint that has real signal even if most features are still 0', async () => {
    const machine = { _id: new Types.ObjectId() };
    const mostlyZero = { features: FEATURE_NAMES.map((name) => (name === 'active_alarm_count' ? 1 : 0)), isEmpty: false };
  ...
  it('pools checkpoints across every machine', async () => {
    ...
    expect(machineIds).toEqual(new Set([machineA._id.toString(), machineB._id.toString()]));
  });
  ...
  it('spaces checkpoints 7 days apart, walking backward from `now`', async () => {
    ...
    const sortedDates = samples.map((s) => s.asOfDate.getTime()).sort((a, b) => b - a);
