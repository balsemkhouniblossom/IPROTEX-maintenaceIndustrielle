...
describe('FeatureExtractionService', () => {
  ...
  function buildService(overrides: {
    ...
    const faultEventModel = {
      ...
      countDocuments: jest.fn().mockReturnValue(execResolves(overrides.recentFaultCount ?? 0)),
    };

    const workOrderModel = {
      countDocuments: jest.fn().mockImplementation((filter: Record<string, unknown>) => {
        if (filter.status === 'overdue') return execResolves(overrides.overdueCount ?? 0);
        if (filter.type_maintenance === 'corrective') return execResolves(overrides.correctiveCount ?? 0);
        if (filter.type_maintenance === 'preventive') return execResolves(overrides.preventiveCount ?? 0);
        return execResolves(0);
      }),
      distinct: jest.fn().mockReturnValue(execResolves(overrides.workOrderIds ?? [])),
    };

    const interventionReportModel = { find: findSortStub(overrides.interventionReports ?? []) };
    const moduleModel = { distinct: jest.fn().mockReturnValue(execResolves(overrides.moduleIds ?? [])) };
    const maintenancePlanModel = {
      countDocuments: jest.fn().mockReturnValue(execResolves(overrides.activePlanCount ?? 0)),
    };
  ...
  it('returns a feature vector matching FEATURE_NAMES length, all zero except the "no recent intervention" sentinel when there is no data', async () => {
    ...
    const daysSinceIndex = FEATURE_NAMES.indexOf('days_since_last_intervention');
  ...
  it('marks isEmpty false as soon as any one data source has activity, even if every other source is empty', async () => {
    const service = buildService({ telemetry: [{ metrics: { temperature: 70 } }] });
    const result = await service.buildFeatureVector(machineId, asOfDate);
  ...
  it('counts telemetry samples and distinct metric keys within the telemetry window', async () => {
    ...
    const result = await service.buildFeatureVector(machineId, asOfDate);
    const sampleCount = result.features[FEATURE_NAMES.indexOf('telemetry_sample_count')];
    const keyCount = result.features[FEATURE_NAMES.indexOf('telemetry_metric_key_count')];
  ...
  it('computes a high max z-score when one telemetry reading is a wild outlier within its own metric key', async () => {
    ...
    const result = await service.buildFeatureVector(machineId, asOfDate);
    const maxZScore = result.features[FEATURE_NAMES.indexOf('telemetry_max_zscore')];
  ...
  it('counts active alarms and separates critical ones', async () => {
    ...
    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('active_alarm_count')]).toBe(3);
    expect(result.features[FEATURE_NAMES.indexOf('critical_alarm_count')]).toBe(2);
    expect(result.features[FEATURE_NAMES.indexOf('recent_fault_event_count')]).toBe(5);
  });

  it('reports corrective/preventive/overdue work order counts independently', async () => {
    const service = buildService({ correctiveCount: 4, preventiveCount: 2, overdueCount: 1 });

    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('corrective_work_order_count')]).toBe(4);
    expect(result.features[FEATURE_NAMES.indexOf('preventive_work_order_count')]).toBe(2);
    expect(result.features[FEATURE_NAMES.indexOf('overdue_work_order_count')]).toBe(1);
  });

  it('reports zero active maintenance plans when the machine has no modules', async () => {
    const service = buildService({ moduleIds: [] });
    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('active_maintenance_plan_count')]).toBe(0);
  });

  it('reports the configured active maintenance plan count when the machine has modules', async () => {
    const service = buildService({ moduleIds: [new Types.ObjectId()], activePlanCount: 3 });
    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('active_maintenance_plan_count')]).toBe(3);
  });

  it('uses the sentinel "no recent intervention" value when there are no completed reports', async () => {
    const service = buildService({ workOrderIds: [new Types.ObjectId()], interventionReports: [] });
    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('days_since_last_intervention')]).toBe(365);
    expect(result.measuredFacts.some((f) => f.includes('No completed intervention'))).toBe(true);
  });
  ...
  it('computes average resolution days and days since last intervention from reports', async () => {
    ...
    const result = await service.buildFeatureVector(machineId, asOfDate);
    expect(result.features[FEATURE_NAMES.indexOf('avg_resolution_days')]).toBeCloseTo(1, 5);
    expect(result.features[FEATURE_NAMES.indexOf('days_since_last_intervention')]).toBeCloseTo(1, 5);
  });
