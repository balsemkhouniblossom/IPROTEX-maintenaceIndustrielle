...
describe('DeviceAuthService', () => {
  ...
  describe('verifyCredentials', () => {
    it('rejects a malformed key with no prefix separator', async () => {
      await expect(service.verifyCredentials('DEV-1', 'not-a-valid-key')).rejects.toThrow(
    ...
    it('rejects an unknown device id', async () => {
      ...
      await expect(service.verifyCredentials('DEV-1', 'prefix123.secret')).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('rejects a deactivated device even with the correct key', async () => {
      ...
      await expect(service.verifyCredentials('DEV-1', generated.rawKey)).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('accepts the correct key for a real, active device and returns it', async () => {
      const generated = await service.generateApiKey();
      const device = { is_active: true, api_key_hash: generated.keyHash, device_id: 'DEV-1' };
      deviceModel.findOne.mockReturnValue({ exec: jest.fn().mockResolvedValue(device) });
  ...
  describe('getDeviceOrThrow', () => {
    it('rejects an invalid Mongo id', async () => {
      await expect(service.getDeviceOrThrow('not-an-id')).rejects.toThrow(UnauthorizedException);
    });

    it('rejects a missing device', async () => {
      deviceModel.findById.mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
      await expect(service.getDeviceOrThrow(new Types.ObjectId().toString())).rejects.toThrow(
        UnauthorizedException,
      );
    });
    ...
    it('returns an active device', async () => {
      ...
      deviceModel.findById.mockReturnValue({ exec: jest.fn().mockResolvedValue(device) });
      const result = await service.getDeviceOrThrow(new Types.ObjectId().toString());
      expect(result).toBe(device);
    });
  });
});
