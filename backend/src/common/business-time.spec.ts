...
describe('business-time', () => {
  ...
  describe('assertValidTimezone', () => {
    it('accepts a real IANA timezone name', () => {
      expect(() =>
        businessTime.assertValidTimezone('America/New_York'),
      ).not.toThrow();
    });

    it('throws for an unrecognized zone name', () => {
      expect(() =>
        businessTime.assertValidTimezone('Not/A_Real_Zone'),
      ).toThrow();
    });
  });

  describe('startOfBusinessDay / endOfBusinessDay', () => {
    it('parses date-only inputs as business-timezone midnight before storing UTC', () => {
      const result = businessTime.parseBusinessDateInput(
        '2026-03-08',
        'America/New_York',
      );
    ...
    it('computes start of day in the business timezone, not server-local/UTC', () => {
      ...
      const result = businessTime.startOfBusinessDay(
    ...
    it('computes end of day as the last millisecond before the next business day starts', () => {
      const result = businessTime.endOfBusinessDay(
  ...
  describe('addBusinessMonths', () => {
    it('adds business months and clamps overflowing days to the target month length', () => {
      ...
      const result = businessTime.addBusinessMonths(
  ...
  describe('addBusinessYears', () => {
    it('keeps yearly recurrence in the configured timezone', () => {
      const result = businessTime.addBusinessYears(
        new Date('2026-07-14T23:30:00.000Z'),
        1,
        tz,
      );
      expect(result.toISOString()).toBe('2027-07-14T23:30:00.000Z');
    });

    it('clamps leap-day yearly recurrence to February 28 on non-leap years', () => {
      const result = businessTime.addBusinessYears(
        new Date('2028-02-29T08:00:00.000Z'),
        1,
        tz,
      );
      expect(result.toISOString()).toBe('2029-02-28T08:00:00.000Z');
    });
  });
  ...
  describe('startOfBusinessWeek', () => {
    it('finds the Monday start of the business week', () => {
      ...
      const result = businessTime.startOfBusinessWeek(
  ...
  describe('startOfBusinessMonth / startOfBusinessYear', () => {
    it('finds the first day of the business month', () => {
      const result = businessTime.startOfBusinessMonth(
        new Date('2026-07-16T12:00:00.000Z'),
        tz,
      );
    ...
    it('finds the first day of the business year', () => {
      const result = businessTime.startOfBusinessYear(
        new Date('2026-07-16T12:00:00.000Z'),
        tz,
      );
