...
describe('simple CRUD services coverage', () => {
  ...
  it('covers catalogue CRUD and pagination wrappers', async () => {
    ...
    model.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'catalogue-id', designation: 'Bearing' }),
    );
    model.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'catalogue-id' }),
    );
    const service = new CataloguesService(model as never);

    await expect(
      service.create({ designation: 'Bearing' } as never),
    ...
    await expect(service.findOne('catalogue-id')).resolves.toMatchObject({
      _id: 'catalogue-id',
    });
    await expect(
      service.update('catalogue-id', { designation: 'Bearing' } as never),
    ).resolves.toMatchObject({
      designation: 'Bearing',
    });
    await expect(service.remove('catalogue-id')).resolves.toMatchObject({
      _id: 'catalogue-id',
    });
  });
  ...
  it('covers lubricants CRUD and pagination wrappers', async () => {
    ...
    model.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'lubricant-id', name: 'Grease' }),
    );
    model.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'lubricant-id' }),
    );
    const service = new LubrifiantsService(model as never);

    await expect(
      service.create({ name: 'Grease' } as never),
    ).resolves.toMatchObject({ name: 'Grease' });
    await expect(service.findAll(1, 10, 0)).resolves.toMatchObject({
      items: [{ _id: 'lubricant-id' }],
      totalItems: 1,
      totalPages: 1,
    });
    await expect(service.findOne('lubricant-id')).resolves.toMatchObject({
      _id: 'lubricant-id',
    });
    await expect(
      service.update('lubricant-id', { name: 'Grease' } as never),
    ).resolves.toMatchObject({
      name: 'Grease',
    });
    await expect(service.remove('lubricant-id')).resolves.toMatchObject({
      _id: 'lubricant-id',
    });
  });
  ...
  it('covers measurements filtering, population, and not-found branches', async () => {
    ...
    model.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'measurement-id', status: 'ok' }),
    );
    model.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'measurement-id' }),
    );
    const service = new MesuresService(model as never);

    await expect(service.create({ value: 42 } as never)).resolves.toMatchObject(
      { value: 42 },
    );
    await expect(
      service.findAll(1, 20, 0, 'sensor-id', 'warning'),
    ).resolves.toMatchObject({
    ...
    await expect(service.findOne('measurement-id')).resolves.toMatchObject({
      _id: 'measurement-id',
    });
    await expect(
      service.update('measurement-id', { status: 'ok' }),
    ).resolves.toMatchObject({
      status: 'ok',
    });
    await expect(service.remove('measurement-id')).resolves.toMatchObject({
      _id: 'measurement-id',
    });
  ...
  it('covers KPI CRUD, machine population, and pagination wrappers', async () => {
    ...
    await expect(service.findOne('kpi-id')).resolves.toMatchObject({
      _id: 'kpi-id',
    });
    expect(findByIdQuery.populate).toHaveBeenCalledWith('machine_id');
    await expect(
      service.update('kpi-id', { valeur: 95 } as never),
    ).resolves.toMatchObject({ valeur: 95 });
    await expect(service.remove('kpi-id')).resolves.toMatchObject({
      _id: 'kpi-id',
    });
  });
  ...
  it('covers module-pieces filtering, population, and not-found branches', async () => {
    ...
    model.findById.mockReturnValue(findByIdQuery);
    model.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'module-piece-id', quantite: 2 }),
    );
    model.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'module-piece-id' }),
    );
    const service = new ModulePiecesService(model as never);

    await expect(
      service.create({ quantite: 1 } as never),
    ).resolves.toMatchObject({ quantite: 1 });
    await expect(
      service.findAll(1, 5, 0, 'module-type-id'),
    ).resolves.toMatchObject({
    ...
    await expect(service.findOne('module-piece-id')).resolves.toMatchObject({
      _id: 'module-piece-id',
    });
    expect(findByIdQuery.populate).toHaveBeenCalledWith('mod_type_id');
    expect(findByIdQuery.populate).toHaveBeenCalledWith('part_id');
    await expect(
      service.update('module-piece-id', { quantite: 2 } as never),
    ).resolves.toMatchObject({ quantite: 2 });
    await expect(service.remove('module-piece-id')).resolves.toMatchObject({
      _id: 'module-piece-id',
    });
  ...
  it('covers pannes and panne-solutions CRUD wrappers', async () => {
    ...
    panneModel.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'panne-id', description: 'Updated' }),
    );
    ...
    await expect(pannesService.findAll(1, 10, 0)).resolves.toMatchObject({
      totalItems: 2,
      totalPages: 1,
    });
    await expect(pannesService.findOne('panne-id')).resolves.toMatchObject({
      _id: 'panne-id',
    });
    await expect(
      pannesService.update('panne-id', { description: 'Updated' }),
    ).resolves.toMatchObject({
      description: 'Updated',
    });
    await expect(pannesService.remove('panne-id')).resolves.toMatchObject({
      _id: 'panne-id',
    });
    ...
    const solutionUpdateQuery = createQuery({
      _id: 'solution-id',
      solution: 'Reset',
    });
    ...
    solutionModel.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'solution-id' }),
    );
    const solutionsService = new PanneSolutionsService(solutionModel as never);

    await expect(
      solutionsService.create({ solution: 'Replace' } as never),
    ).resolves.toMatchObject({
      solution: 'Replace',
    });
    await expect(solutionsService.findAll(1, 10, 0)).resolves.toMatchObject({
      totalItems: 1,
    });
    expect(solutionFindQuery.populate).toHaveBeenCalledWith('panne_id');
    await expect(
      solutionsService.findOne('solution-id'),
    ).resolves.toMatchObject({ _id: 'solution-id' });
    expect(solutionFindByIdQuery.populate).toHaveBeenCalledWith('panne_id');
    await expect(
      solutionsService.update('solution-id', { solution: 'Reset' } as never),
    ).resolves.toMatchObject({
      solution: 'Reset',
    });
    expect(solutionUpdateQuery.populate).toHaveBeenCalledWith('panne_id');
    await expect(solutionsService.remove('solution-id')).resolves.toMatchObject(
      { _id: 'solution-id' },
    );
  });
  ...
  it('covers machine type sequence assignment, response mapping, and dependency-protected delete', async () => {
    ...
    await expect(service.create({ name: 'Press' })).resolves.toMatchObject({
      _id: 'machine-type-id',
      type_id: 12,
      name: 'Press',
    });
    ...
    await expect(service.findOne('machine-type-id')).resolves.toMatchObject({
      name: 'Press',
    });
    await expect(
      service.update('machine-type-id', { name: 'Updated press' }),
    ).resolves.toMatchObject({
      name: 'Updated press',
    });
    await expect(service.remove('machine-type-id')).resolves.toMatchObject({
      name: 'Press',
    });

    machineModel.exists.mockReturnValueOnce(
      createExistsQuery({ _id: 'machine-id' }),
    );
    ...
    await expect(service.remove('machine-type-id')).rejects.toMatchObject({
      status: 409,
    });
  });
  ...
  it('covers module type CRUD and dependency-protected delete', async () => {
    ...
    moduleTypeModel.find.mockReturnValue(
      createQuery([{ _id: 'module-type-id' }]),
    );
    moduleTypeModel.countDocuments.mockReturnValue(createQuery(4));
    moduleTypeModel.findById.mockReturnValue(
      createQuery({ _id: 'module-type-id' }),
    );
    moduleTypeModel.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'module-type-id', nom_module: 'Motor' }),
    );
    moduleTypeModel.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'module-type-id' }),
    );
    moduleModel.exists.mockReturnValue(createExistsQuery(null));
    modulePiecesModel.exists.mockReturnValue(createExistsQuery(null));
    const service = new ModuleTypesService(
      moduleTypeModel as never,
      moduleModel as never,
      modulePiecesModel as never,
    );

    await expect(
      service.create({ nom_module: 'Motor' } as never),
    ).resolves.toMatchObject({ nom_module: 'Motor' });
    await expect(service.findAll(2, 2, 2)).resolves.toMatchObject({
      totalItems: 4,
      totalPages: 2,
    });
    await expect(service.findOne('module-type-id')).resolves.toMatchObject({
      _id: 'module-type-id',
    });
    await expect(
      service.update('module-type-id', { nom_module: 'Motor' }),
    ).resolves.toMatchObject({
      nom_module: 'Motor',
    });
    await expect(service.remove('module-type-id')).resolves.toMatchObject({
      _id: 'module-type-id',
    });

    moduleModel.exists.mockReturnValueOnce(
      createExistsQuery({ _id: 'module-id' }),
    );
    modulePiecesModel.exists.mockReturnValueOnce(createExistsQuery(null));
    await expect(service.remove('module-type-id')).rejects.toMatchObject({
      status: 409,
    });
  });
  ...
  it('covers module CRUD, populated reads, and all dependency checks before delete', async () => {
    ...
    moduleModel.findByIdAndUpdate.mockReturnValue(
      createQuery({ _id: 'module-id', localisation: 'Line 1' }),
    );
    moduleModel.findByIdAndDelete.mockReturnValue(
      createQuery({ _id: 'module-id' }),
    );
    ...
    await expect(
      service.create({ localisation: 'Line 1' } as never),
    ).resolves.toMatchObject({
      localisation: 'Line 1',
    });
    await expect(service.findAll(1, 3, 0)).resolves.toMatchObject({
      totalItems: 3,
      totalPages: 1,
    });
    expect(findQuery.populate).toHaveBeenCalledWith('machine_id');
    ...
    await expect(service.findOne('module-id')).resolves.toMatchObject({
      _id: 'module-id',
    });
    expect(findByIdQuery.populate).toHaveBeenCalledWith('machine_id');
    await expect(
      service.update('module-id', { localisation: 'Line 1' }),
    ).resolves.toMatchObject({
      localisation: 'Line 1',
    });
    await expect(service.remove('module-id')).resolves.toMatchObject({
      _id: 'module-id',
    });

    workOrderModel.exists.mockReturnValueOnce(
      createExistsQuery({ _id: 'work-order-id' }),
    );
    await expect(service.remove('module-id')).rejects.toMatchObject({
      status: 409,
    });
  });
});
