  ...
  function buildProvider(opts: {
    ...
    const documentModel = {
      find: jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue(execResolves(opts.documents ?? [])),
      }),
    };
    const maintenancePlanModel = {
      find: jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue(execResolves(opts.plans ?? [])),
      }),
    };
    ...
    const userModel = {
      find: jest.fn().mockReturnValue({
        select: jest.fn().mockReturnValue(execResolves(opts.users ?? [])),
      }),
    };
