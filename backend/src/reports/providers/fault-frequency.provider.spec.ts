...
describe('FaultFrequencyReportProvider', () => {
  ...
  function buildProvider(aggregateRows: unknown[]) {
    const faultEventModel = {
      aggregate: jest.fn().mockReturnValue({ exec: jest.fn().mockResolvedValue(aggregateRows) }),
    };
    ...
    const provider = new FaultFrequencyReportProvider(faultEventModel as never, documentAccessService as never);
  ...
  it('ranks fault codes by occurrence count, exposing critical occurrences and last-raised', async () => {
    const { provider } = buildProvider([
      { _id: 'E-1', count: 12, criticalCount: 3, lastRaisedAt: new Date('2026-06-01T00:00:00Z') },
      { _id: 'E-2', count: 5, criticalCount: 0, lastRaisedAt: new Date('2026-05-01T00:00:00Z') },
    ]);

    const dataset = await provider.buildDataset({}, actor);

    expect(dataset.rows).toEqual([
      expect.objectContaining({ fault_code: 'E-1', occurrences: 12, critical_occurrences: 3 }),
      expect.objectContaining({ fault_code: 'E-2', occurrences: 5, critical_occurrences: 0 }),
    ]);
  ...
  it('scopes the aggregation match stage by machine and date range', async () => {
    ...
    await provider.buildDataset({ machineId: new Types.ObjectId().toString(), dateFrom, dateTo }, actor);

    const [pipeline] = faultEventModel.aggregate.mock.calls[0];
    expect(pipeline[0].$match.machine_id).toBeDefined();
    expect(pipeline[0].$match.raised_at).toEqual({ $gte: dateFrom, $lt: dateTo });
  });
