...
describe('Maintenance Work Order with OT Pieces and live Stock (e2e)', () => {
  ...
  it('creates a work order and test stock item, consumes 3 units through an OT Piece, then restores them', async () => {
    const machineType = await machineTypes.create({
      type_id: 750102,
      name: 'Braiding test machine',
    });
    const machine = await machines.create({
      machine_id: 'BRAID-TEST-01',
      type_id: machineType._id,
      serial_no: 'BRAID-TEST-SN',
      status: 'active',
    });
    const admin = await users.create({
      user_id: 'ADMIN-OT-PIECE-TEST',
      nom_complet: 'Inventory Test Admin',
      email: 'inventory-test-admin@example.test',
      password: 'x',
      role: 'admin',
      is_active: true,
      is_verified: true,
    ...
    const technician = await users.create({
      user_id: 'TECH-OT-PIECE-TEST',
      nom_complet: 'Inventory Test Technician',
      email: 'inventory-test-technician@example.test',
      password: 'x',
      role: 'technician',
      is_active: true,
      is_verified: true,
    ...
    const catalogueResponse = await request(app.getHttpServer())
      ...
      .send({
        part_id: 'TEST-BEARING-6203',
        nom_piece: 'Test drive bearing 6203',
        ref_constructeur: 'SKF-6203-2Z',
        fabricant: 'SKF',
        categorie_piece: 'Bearings',
      })
      .expect(201);
    ...
    await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${token}`)
      .send({
        stock_id: 'STK-TEST-BEARING-6203',
        part_id: partId,
        quantite_en_stock: 10,
        seuil_alerte_stock: 2,
        quantite_minimale: 2,
        emplacement: 'Test shelf A1',
      })
      .expect(201);
    ...
    const workOrderResponse = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${token}`)
      .send({
        ot_id: 'WO-STOCK-EXAMPLE-001',
        machine_id: machine._id.toString(),
        technician_id: technician._id.toString(),
        description: 'Replace the test drive bearing',
        type_maintenance: 'corrective',
        status: 'in_progress',
        priorite: 'medium',
        date_created: '2026-09-14T08:00:00.000Z',
    ...
    expect(
      await stocks.findOne({ stock_id: 'STK-TEST-BEARING-6203' }).lean(),
    ).toMatchObject({ quantite_en_stock: 7, quantite_reservee: 0 });

    await request(app.getHttpServer())
      .delete(`/ot-pieces/${otPieceResponse.body._id as string}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);

    expect(
      await stocks.findOne({ stock_id: 'STK-TEST-BEARING-6203' }).lean(),
    ).toMatchObject({ quantite_en_stock: 10, quantite_reservee: 0 });
    expect(
      await catalogues.findOne({ part_id: 'TEST-BEARING-6203' }).lean(),
    ).toMatchObject({ nom_piece: 'Test drive bearing 6203' });
  });
});
