...
> Restored and retained as the practical Phase 2 acceptance checklist. Run it
> only against the dedicated staging database/storage and the approved release
> candidate. Never substitute production URLs or identities.
...
- [ ] Anonymous and incorrect-token requests to FastAPI `/v1/*` return `401`;
      the NestJS staging identity using the shared staging-only
      `AI_SERVICE_TOKEN` succeeds. Never paste the token into this checklist.
- [ ] `npm run verify:uat-staging` passes after the guarded synthetic seed.
